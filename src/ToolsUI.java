...
public class ToolsUI{
    ...
    public ToolsUI(){
        ...
        tools_panel.setSize(600,300);
        tools_panel.setMaximumSize(new Dimension(600,300));
        tools_panel.setMinimumSize(new Dimension(600,300));
        tools_panel.setPreferredSize(new Dimension(600,300));

        boolean[] canEdit = {true, true, true, true, true, true};
        service = new Service();
        service.table(1,canEdit, "");
        ...
        Btn_Save.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent actionEvent) {
                JSONArray final_ja_words = new JSONArray();
                try {
                    for(int i=0;i< table1.getRowCount();i++){
                        ...
                        final_ja_words.put(i, new JSONObject().put(key, val));
                    }
        ...
        table1.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent evt) {
                ...
                add_warning_icon_to_col();
                message_warning_ew(evt, ja_equal_words, 4);
                message_warning_ew(evt, ja_equal_words2, 5);
        ...
        Btn_Back.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent actionEvent) {
                ...
                if(ks>=0) {
                    ...
                    service.table(2, canEdit, storageTables.val[ks]);
    ...
    public void add_warning_icon_to_col(){
        ...
        int n = table1.getRowCount()-1;
        do {
            add_equal_el_to_ja(list_val_from_col2, ja_equal_words, n, set, im, 2,4);
            add_equal_el_to_ja(list_val_from_col3, ja_equal_words2, n, set, im, 3,5);
        }while(--n>=0);
    }


    public void add_equal_el_to_ja(ArrayList<String> list_val_from_col, JSONArray ja, int n, TreeSet set, ImageIcon im, int tc, int tci){
        list_val_from_col.add(table1.getValueAt(n, getCurrentColumnIndex(tc)).toString());
        Object obj = table1.getValueAt(n, getCurrentColumnIndex(tc));
        if(!set.add(obj)){
            table1.setValueAt(im, n, getCurrentColumnIndex(tci));
            table1.setValueAt(im, table1.getRowCount()- 1-list_val_from_col.indexOf(obj), getCurrentColumnIndex(tci));
            ja_equal_error.put(2,2);
            ja.put(table1.getRowCount()- 1-list_val_from_col.indexOf(obj), obj);
            ja.put(n, obj);
        }
    }

    public void message_warning_ew(MouseEvent evt, JSONArray ja_ew, int col_v){
        int row = table1.rowAtPoint(evt.getPoint());
        int col = table1.columnAtPoint(evt.getPoint());
